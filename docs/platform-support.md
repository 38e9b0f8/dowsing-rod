...
The npm launcher requires Node 18+, and the NuGet tool requires a supported .NET 8 runtime, so those runtimes can impose a higher OS floor than the native scanner. The Python wheels use the `manylinux2014` / glibc 2.17 baseline on Linux.
