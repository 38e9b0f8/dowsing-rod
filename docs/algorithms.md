...
This document describes the current `0.3.0` analysis pipeline. The implementation is deterministic and local-first: every phase runs in-process over source files and produces serializable evidence for renderers and downstream tools.
...
.eggs, *.egg-info, .dowsing-rod-cache, vendor, third_party, 3rdparty, external,
deps, generated directories, snapshots, baselines, generated source names, protobuf output, and minified JavaScript
```

User exclusions are added as override globs. When inclusions are provided they form an allowlist, and are applied after exclusions so they can opt paths back in.

Supported extensions are `.py`, `.pyi`, `.js`, `.jsx`, `.mjs`, `.cjs`, `.ts`, `.tsx`, `.mts`, `.cts`, C/C++ source and headers, `.cs`, `.rs`, `.v`, `.vh`, `.sv`, `.svh`, `.vhd`, and `.vhdl`. Single-file scans bypass the directory walker.
...
Python uses `rustpython-parser`; JavaScript, TypeScript, C, C++, C#, Rust, Verilog/SystemVerilog, and VHDL use embedded Tree-sitter grammars. No compiler, language server, service, or runtime grammar download is required. Parse failures are converted into `ParseError` values with file, row, column, and parser message. Diagnostics are bounded per file and in the final report so generated or macro-heavy input cannot flood a scan. The scanner normally keeps going when a file fails to parse. With `fail_on_error = true`, the core API returns an error after collecting parse errors. HDL grammar failures produce an outline of local declarations for discovery instead of a syntax diagnostic; macro expansions and foreign DPI declarations are omitted. The tool does not replace an HDL compiler, linter, simulator, or synthesizer. Rust analysis does not expand macros or evaluate `cfg` attributes: functions generated by macros are invisible, macro token trees are opaque, and mutually exclusive `#[cfg]` copies in the same file can both be extracted.
...
```text
normalized_duplicated_tokens * average_similarity * confidence * log2(cluster_size)
```
